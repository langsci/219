...
To my knowledge, the only extensive study that has been done on phrase-level \isi{prosody} in a Zapotecan language has been the work of Mark Sicoli (2007; 2010). In his PhD dissertation, \textit{A linguistic ethnography of \isi{tone} and voice in a Zapotec region}, Sicoli devotes two chapters to an analysis of \isi{prosody} in Lachix\'{i}o Zapotec (Eastern Zapotec) at both the word level and the phrase level. Although Lachix\'{i}o Zapotec and ZAI are only distantly related, it is not surprising that many of Sicoli's observations with respect to prosodic structure hold for ZAI as well. 
...
The comparative construction currently used in ZAI, with the order adjective-marker-standard, is a construction borrowed from the \ili{Spanish} \textit{m\'{a}s que}. An example is shown in (\ref{comp}):
